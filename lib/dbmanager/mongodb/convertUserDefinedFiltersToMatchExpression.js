...
function convertSqlLikeExpressionToRegExp(likeExpression, isCaseSensitive = false) {
    return new RegExp(`^${likeExpression
        .split(/(\[.+?])/g)
        .map((s, i) => i % 2
        ? s.replace(/\\/g, '\\\\')
        : s.replace(/[-/\\^$*+?.()|[\]{}%_]/g, (m) => {
        ...
        .join('')}$`, isCaseSensitive ? '' : 'i');
}
function getWhereExpression(userDefinedFilter) {
    let exprLeftHandSide;
    ...
    switch (userDefinedFilter.operator) {
        ...
        case 'IN':
            return {
                [userDefinedFilter.fieldName]: {
                    $in: userDefinedFilter.value.map((v) => {
                        return {
                            $where: `function() {
        ...
        case 'NOT IN':
            return {
                [userDefinedFilter.fieldName]: {
                    $nin: userDefinedFilter.value.map((v) => {
                        return {
                            $where: `function() {
                  return ${exprLeftHandSide} == ${v};
                }`
...
function convertUserDefinedFiltersToMatchExpression(userDefinedFilters) {
    return userDefinedFilters.reduce((matchExpressions, userDefinedFilter) => {
        if (userDefinedFilter.operator === 'OR' && userDefinedFilter.orFilters) {
            return {
                $or: userDefinedFilter.orFilters.map((orFilter) => convertUserDefinedFiltersToMatchExpression([orFilter]))
            };
        }
        if (!userDefinedFilter.fieldName) {
...
exports.default = convertUserDefinedFiltersToMatchExpression;
//# sourceMappingURL=convertUserDefinedFiltersToMatchExpression.js.map