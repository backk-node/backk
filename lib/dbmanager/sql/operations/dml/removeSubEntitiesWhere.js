...
async function removeSubEntitiesByFilters(dbManager, filters, subEntitiesJsonPath, EntityClass, entityPreHooks, postHook, postQueryOperations) {
    EntityClass = dbManager.getType(EntityClass);
    let didStartTransaction = false;
    try {
        didStartTransaction = await tryStartLocalTransactionIfNeeded_1.default(dbManager);
        const [currentEntity] = await getEntityByFilters_1.default(dbManager, filters, EntityClass, { postQueryOperations }, true, true);
        if (currentEntity) {
...
exports.default = removeSubEntitiesByFilters;
//# sourceMappingURL=removeSubEntitiesWhere.js.map