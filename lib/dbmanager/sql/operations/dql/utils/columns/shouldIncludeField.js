...
function shouldIncludeField(fieldName, fieldPath, { includeResponseFields, excludeResponseFields }) {
    let shouldIncludeField = true;
    const fullFieldPath = fieldPath.endsWith('.') || !fieldPath ? fieldPath + fieldName : fieldPath + (fieldName ? '.' : '') + fieldName;
    if (includeResponseFields && includeResponseFields.length > 0) {
        shouldIncludeField = !!includeResponseFields.find((includeResponseField) => {
            ...
            else if (includeResponseField.length > fullFieldPath.length &&
    ...
    if (excludeResponseFields && excludeResponseFields.length > 0) {
        const shouldExcludeField = !!excludeResponseFields.find((excludeResponseField) => excludeResponseField === fullFieldPath.slice(0, excludeResponseField.length));
        shouldIncludeField = shouldExcludeField ? false : shouldIncludeField;
    }
    return shouldIncludeField;
}
exports.default = shouldIncludeField;
//# sourceMappingURL=shouldIncludeField.js.map