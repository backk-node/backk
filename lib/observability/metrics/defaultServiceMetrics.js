...
const defaultPrometheusMeter_1 = __importDefault(require("./defaultPrometheusMeter"));
class DefaultServiceMetrics {
    constructor(meter) {
        this.meter = meter;
        this.processingTimeInSecsBuckets = [
            0.001,
            0.005,
            0.01,
            0.05,
        ...
        this.defaultLabels = { pid: process.pid.toString() };
        ...
        this.serviceFunctionCallCachedResponsesCounter = meter.createCounter('cached_service_function_call_responses', {
        ...
        this.serviceFunctionProcessingTimeCounter = meter.createCounter('service_function_processing_time', {
            description: 'Count of service function calls where service function processing time is in processing time bucket'
        });
        this.dbOperationProcessingTimeCounter = meter.createCounter('db_operation_processing_time', {
            description: 'Count of database operations where database operation processing time is in processing time bucket'
        });
        ...
        this.syncRemoteServiceHttp5xxErrorResponseCounter = meter.createCounter('sync_remote_service_call_http_5xx_error_responses', {
        ...
        this.syncRemoteServiceCallAuthFailureCounter = meter.createCounter('sync_remote_service_call_auth_failures', {
    ...
    incrementServiceFunctionCallsByOne(serviceFunction) {
    ...
    incrementDbOperationErrorsByOne(dbManagerType, dbHost) {
        this.dbOperationErrorCounter.bind({ ...this.defaultLabels, dbManagerType, dbHost }).add(1);
    }
    recordDbFailureDurationInSecs(dbManagerType, dbHost, failureDurationInSecs) {
        this.dbFailureDurationInSecsRecorder
            .bind({ ...this.defaultLabels, dbManagerType, dbHost })
            .record(failureDurationInSecs);
    }
    incrementServiceFunctionProcessingTimeInSecsBucketCounterByOne(serviceFunction, processingTimeInSecs) {
        const foundProcessingTimeInSecsBucket = this.processingTimeInSecsBuckets.find((processingTimeInSecsBucket) => processingTimeInSecs <= processingTimeInSecsBucket);
    ...
    incrementDbOperationProcessingTimeInSecsBucketCounterByOne(dbManagerType, dbHost, processingTimeInSecs) {
        const foundProcessingTimeInSecsBucket = this.processingTimeInSecsBuckets.find((processingTimeInSecsBucket) => processingTimeInSecs <= processingTimeInSecsBucket);
    ...
    recordRedisConsumerQueueLength(queueLength) {
        this.redisConsumerQueueLengthRecorder.bind(this.defaultLabels).record(queueLength);
    }
    incrementServiceFunctionCallCacheHitCounterByOne(serviceFunction) {
        this.serviceFunctionCallCacheHitCounter.bind({ ...this.defaultLabels, serviceFunction }).add(1);
    }
    incrementServiceFunctionCallCachedResponsesCounterByOne(serviceFunction) {
        this.serviceFunctionCallCachedResponsesCounter.bind({ ...this.defaultLabels, serviceFunction }).add(1);
    }
}
exports.default = new DefaultServiceMetrics(defaultPrometheusMeter_1.default);
//# sourceMappingURL=defaultServiceMetrics.js.map