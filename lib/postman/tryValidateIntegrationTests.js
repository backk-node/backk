...
function tryValidateIntegrationTests(integrationTests, servicesMetadata) {
    integrationTests.forEach((integrationTest, index) => {
        ...
        const foundInvalidKey = Object.keys(integrationTest.testTemplate).find((key) => key !== 'after' &&
            key !== 'before' &&
            key !== 'at' &&
        ...
        if (integrationTest.testTemplate) {
            if (!integrationTest.testTemplate.responseStatusCode) {
                integrationTest.testTemplate.responseStatusCode = constants_1.HttpStatusCodes.SUCCESS;
            }
        }
        if (integrationTest.tests) {
            integrationTest.tests.forEach((test) => {
                const foundInvalidKey = Object.keys(test).find((key) => key !== 'testName' && key !== 'testProperties');
                if (foundInvalidKey) {
        ...
        if (integrationTest.testTemplate.before) {
            ...
            const serviceMetadata = servicesMetadata.find((serviceMetadata) => serviceMetadata.serviceName.toLowerCase() === serviceName.toLowerCase());
            const functionMetadata = serviceMetadata === null || serviceMetadata === void 0 ? void 0 : serviceMetadata.functions.find((func) => func.functionName.toLowerCase() === functionName.toLowerCase());
...
exports.default = tryValidateIntegrationTests;
//# sourceMappingURL=tryValidateIntegrationTests.js.map