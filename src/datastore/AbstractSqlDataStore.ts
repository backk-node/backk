...
export default abstract class AbstractSqlDataStore extends AbstractDataStore {
  ...
  async isDbReady(): Promise<boolean> {
    try {
      ...
      try {
        const createTableStatement = `CREATE TABLE IF NOT EXISTS ${this.getSchema().toLowerCase()}.__backk_db_initialization (microserviceversion VARCHAR(64) PRIMARY KEY NOT NULL UNIQUE, isinitialized TINYINT, createdattimestamp ${this.getTimestampType()})`;
