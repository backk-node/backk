...
export default function log(
  ...
  const now = new Date();
  const spanId = tracerProvider.getTracer('default').getCurrentSpan()?.context().spanId;
  ...
  if (severityNumber >= minLoggingSeverityNumber) {
    const logEntry: LogEntry = {
      ...
      SpanId: spanId,
      TraceFlags: tracerProvider.getTracer('default').getCurrentSpan()?.context().traceFlags,
